...
def build_quote_from_form(quote: Quote | None = None) -> Quote | None:
    ...
    requested_status = request.form.get("statut", "Brouillon").strip() or "Brouillon"
    if action == "save_quote" and requested_status == "Brouillon":
        requested_status = "Envoye"

    allowed_statuses = {"Brouillon", "Envoye", "Accepte", "Refuse"}
    if requested_status not in allowed_statuses:
        return None

    if current_user.role == "employe" and requested_status not in {"Brouillon", "Envoye"}:
        requested_status = "Envoye"
    ...
    for designation_raw, quantity_raw, unit_price_raw in zip(designations, quantities, unit_prices):
        ...
        if not designation or quantite <= 0 or prix_unitaire < 0:
            continue
        rows.append(
            {
